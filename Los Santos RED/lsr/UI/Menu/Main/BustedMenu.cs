...
public class BustedMenu : ModUIMenu
{
    ...
    private void AddServeSentence()
    {
        if (Player.Incarceration == null || !Player.Incarceration.HasPrisonAvailable)
        {
            return;
        }
        UIMenuItem serveSentence = new UIMenuItem("Serve Your Sentence", "Get transferred to prison and do your time. Walk out the gate when your sentence is up. Lose guns and drugs.");
        serveSentence.RightBadge = UIMenuItem.BadgeStyle.Alert;
        serveSentence.Activated += (sender, selectedItem) =>
        {
            Menu.Visible = false;
            Player.Incarceration.StartServingNearest();
        };
        Menu.AddItem(serveSentence);
    }
