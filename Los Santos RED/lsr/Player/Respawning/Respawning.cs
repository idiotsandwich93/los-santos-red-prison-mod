...
public class Respawning// : IRespawning
{
    ...
    public void GetBooked(ILocationRespawnable respawnableLocation)
    {
        ...
        if (!searchActivity.IsActive)
        {
            ...
            SkipBooking(respawnableLocation, null);
            return;
        }
        ...
        GameFiber.StartNew(delegate
        {
            try
            {
                ...
                if (shouldSkip)
                {
                    ...
                    SkipBooking(respawnableLocation, bookingActivity);
                    return;
                }
    ...
    private void SkipBooking(ILocationRespawnable respawnableLocation, BookingActivity bookingActivity)
    {
        ...
        {
            bookingActivity.OnSkipBookingPressed();
        }
    ...
    // Prison incarceration entry: clears the busted state and places the player inside the
    // prison interior WITHOUT skipping time/posting bail (the interactive sentence handles time).
    // Mirrors SurrenderToPolice minus the time-skip and bail notification. Fades are handled by
    // the IncarcerationManager so this just resets + teleports.
    public void SendToPrison(ILocationRespawnable prisonLocation)
    {
        ...
        GTANotification impound = ImpoundVehicles();
        ResetPlayer(true, true, false, false, true, false, true, false, false, false, false, false, true, true, false, true, true, false, false, false, true, false, false);// going to prison must not reset gang territory
        Player.PlayerTasks.OnStandardRespawn();
        SetPlayerAtLocation(prisonLocation);
        Vector3 prisonDest = prisonLocation.RespawnLocation != Vector3.Zero ? prisonLocation.RespawnLocation : prisonLocation.EntrancePosition;
        EnsureGroundLoaded(prisonDest, prisonLocation.RespawnHeading);
        if (Settings.SettingsManager.RespawnSettings.ClearIllicitInventoryOnSurrender)
        {
            Player.Inventory.RemoveIllicitInventoryItems();
        }
    ...
    // Teleports the player to an exact coordinate and waits for collision/the MLO to stream in - frozen
    // so they can't fall through into the world below (or the ocean) while it loads.
    private void EnsureGroundLoaded(Vector3 dest, float heading)
    {
        ...
        p.Position = dest;
        p.Heading = heading;
        NativeFunction.CallByName<bool>("FREEZE_ENTITY_POSITION", p, true);
        int waited = 0;
        while (waited < 100 && !NativeFunction.CallByName<bool>("HAS_COLLISION_LOADED_AROUND_ENTITY", p))
        {
            NativeFunction.CallByName<bool>("REQUEST_COLLISION_AT_COORD", dest.X, dest.Y, dest.Z);
            GameFiber.Sleep(50);
            waited++;
        }
        NativeFunction.CallByName<bool>("FREEZE_ENTITY_POSITION", p, false);
    }
    // Release the player to an explicit coordinate (the prison exit), with collision/MLO load handling.
    public void ReleaseToCoordinates(Vector3 pos, float heading)
    {
        EntryPoint.FocusCellX = (int)(pos.X / EntryPoint.CellSize);
        EntryPoint.FocusCellY = (int)(pos.Y / EntryPoint.CellSize);
        EntryPoint.FocusPosition = pos;
        ...
        EnsureGroundLoaded(pos, heading);
    }
    // Move the player to a location's respawn point (e.g. solitary, or back to general population).
    // Reuses the standard placement so streaming/focus is handled. Fades handled by caller.
    public void PlaceAtRespawnLocation(ILocationRespawnable location)
    {
        if (location == null)
        {
            return;
        }
        SetPlayerAtLocation(location);
    }
    // Release from prison: place the player back at the prison gate (entrance). Fades handled by caller.
    public void ReleaseFromPrison(ILocationRespawnable prisonLocation)
    {
        if (prisonLocation == null)
        {
            return;
        }
        Game.LocalPlayer.Character.Position = prisonLocation.EntrancePosition;
        Game.LocalPlayer.Character.Heading = prisonLocation.EntranceHeading;
        EntryPoint.FocusCellX = (int)(prisonLocation.EntrancePosition.X / EntryPoint.CellSize);
        EntryPoint.FocusCellY = (int)(prisonLocation.EntrancePosition.Y / EntryPoint.CellSize);
        EntryPoint.FocusPosition = prisonLocation.EntrancePosition;
        World.ClearSpawned(false);
        Player.Dispatcher.LocationDispatcher.Reset();
        GameTimeLastPlacedAtLocation = Game.GameTime;
        NativeFunction.Natives.CLEAR_PED_TASKS_IMMEDIATELY(Game.LocalPlayer.Character);
    ...
    private void ResetPlayer(bool resetWanted, bool resetHealth, bool resetTimesDied, bool clearWeapons, bool clearCriminalHistory, bool clearInventory, bool clearIntoxication, bool resetGangRelationships, 
        ...
        bool resetBankAccounts, bool resetSavedGame, bool resetPendingMessages, bool resetInteriors, bool resetGambling, bool resetGangTerritory)
    {
        Player.Reset(resetWanted, resetTimesDied, clearWeapons, clearCriminalHistory, clearInventory, clearIntoxication, resetGangRelationships, clearVehicleOwnership, resetCellphone, clearActiveTasks, clearProperties, resetHealth, resetNeeds,
            resetGroup, resetLicenses, resetActivites, resetGracePeriod, resetBankAccounts, resetSavedGame, resetPendingMessages, resetInteriors, resetGambling, false, resetGangTerritory);
    ...
    private void Respawn(bool resetWanted, bool resetHealth, bool resetTimesDied, bool clearWeapons, bool clearCriminalHistory, bool clearInventory, bool clearIntoxication, bool resetGangRelationships, bool clearOwnedVehicles, bool resetCellphone,
        bool clearActiveTasks, bool clearProperties, bool resetNeeds, bool resetGroup, bool resetLicenses, bool resetActivites, bool resetGracePeriod, bool resetBankAccounts, bool resetSavedGame, bool resetPendingMessages, bool resetInteriors, bool resetGambling, bool resetGangTerritory)
    {
        try
        {
            ResurrectPlayer(resetTimesDied);
            ResetPlayer(resetWanted, resetHealth, resetTimesDied, clearWeapons, clearCriminalHistory, clearInventory, clearIntoxication, resetGangRelationships, clearOwnedVehicles, resetCellphone, clearActiveTasks, clearProperties, resetNeeds, resetGroup, resetLicenses, resetActivites, resetGracePeriod, resetBankAccounts, resetSavedGame, resetPendingMessages, resetInteriors, resetGambling, resetGangTerritory);
            Game.HandleRespawn();
            Time.UnPauseTime();
